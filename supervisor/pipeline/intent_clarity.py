...
_SYSTEM: str = (
    ...
    "BE CONSERVATIVE ABOUT ASKING QUESTIONS. Only mark unclear when:\n"
    "  - The user explicitly asks for something open-ended like 'improve this' without scope\n"
    "  - Critical details (file path, target, timeframe) are genuinely missing and unfilled by memory\n"
    "  - The intent is a single word or greeting that could mean anything\n"
    "Do NOT ask clarification when:\n"
    "  - The intent has reasonable specificity even if not perfect\n"
    "  - Memory context fills any gaps\n"
    "  - The request could be executed 'well enough' even if imperfect\n"
    "Reason semantically. Prefer letting the DAG try and fail usefully over blocking early.\n"
