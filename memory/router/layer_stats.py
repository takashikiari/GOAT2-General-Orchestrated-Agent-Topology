"""Per-layer response-time and hit-rate tracking — rolling samples + percentile read helpers."""
...
_SAMPLE_CAP: Final[int] = 1000  # max latency samples kept per layer (ring buffer)


def _percentile(samples: list[float], p: float) -> float:
    """Linear-interpolation percentile over a sorted sample list. Pure — PyO3 candidate."""
    if not samples:
        return 0.0
...
@dataclass
class LayerStats:
    """Accumulated call metrics for one memory layer. Pure snapshot — PyO3 candidate."""
    calls:    int   = 0
    total_ms: float = 0.0
    hits:     int   = 0
    p50_ms:   float = 0.0
    ...
    def hit_rate(self) -> float:
        """Fraction of calls that returned ≥1 result; 0.0 before any calls."""
        return self.hits / self.calls if self.calls else 0.0


class LayerStatsTracker:
    """Mutable registry of LayerStats and latency samples — one entry per memory tier."""
    ...
    def record(self, timing: LayerTiming) -> None:
        """Incorporate one completed-query timing into rolling stats and sample buffer."""
    ...
    def get(self, layer: LayerName) -> LayerStats:
        """Return a copy of LayerStats with current percentiles computed from samples."""
        s     = self._stats[layer]
        samps = sorted(self._samples[layer])
        return LayerStats(
            calls=s.calls, total_ms=s.total_ms, hits=s.hits,
    ...
    def snapshot(self) -> dict[LayerName, LayerStats]:
        """Return a copy of all per-layer stats — safe to pass to pure routing functions."""
