...
class TemporalSearchMixin:
    """timeline / recent / debug_trace methods for MemoryManager."""
    ...
    async def timeline(
        ...
        *, tier: str = "any", limit: int = 100,
        ...
        raw = await gather_tier_list(self._layers, agent_role, tier, limit=limit * 4)
        filtered = sorted(filter_by_time(raw, start_ts, end_ts), key=_TS_KEY, reverse=True)
        return filtered[:limit]

    async def recent(
        self, agent_role: str, *, limit: int = 50, tier: str = "any",
    ) -> list[MemoryEntry]:
        """Return the `limit` most recent entries across tier(s), newest first."""
        entries = await gather_tier_list(self._layers, agent_role, tier, limit=limit * 3)
    ...
    async def debug_trace(
        ...
        """Search each tier separately; return per-tier match counts and top snippets."""
        ...
        for name in ("working", "episodic", "long_term"):
            ...
            tiers[name] = {
                "total": len(raw), "matched": len(matched),
                "entries": [
                    {"key": str(e.key), "created_at": str(e.created_at) or "unknown",
                     "snippet": e.content[:80]}
                    for e in matched[:3]
                ],
            }
        return {"query": query, "range": (start_datetime, end_datetime), "tiers": tiers}
