...
from memory.letta_client import LettaClient, MemoryLayer, letta_client
...
from memory.working_memory import working_memory as _default_working
...
class MemoryManager(MemoryCrudMixin, MemorySearchMixin, MemoryPromoteMixin, TemporalSearchMixin):
    """Single entry-point for all agent memory operations. Agents import only this."""
    ...
    def _get_router(self) -> MemoryRouter:
        """Lazily initialise MemoryRouter on first routed recall; avoids circular import at load time."""
    ...
    async def recall(
        ...
        *, limit: int = 10, tags: list[str] | None = None,
        ...
        """Intelligently route recall through MemoryRouter; falls back to fan-out when tags are set."""
        if tags is not None:
            return await self._fan_out_search(agent_role, query, limit=limit, tags=tags)
        from memory.types import AgentRole
        return await self._get_router().search(AgentRole(agent_role), query, limit=limit)

    async def get_block(self, agent_role: str, label: str) -> str | None:
        """Read a Letta core-memory block (always-in-context named slot) for agent_role."""
        return await self.long_term.get_block(agent_role, label)

    async def set_block(self, agent_role: str, label: str, value: str) -> bool:
        """Write or update a Letta core-memory block. Returns False when Letta is unreachable."""
        return await self.long_term.set_block(agent_role, label, value)

    async def status(self) -> LayerStatus:
        """Concurrent health check; long_term=False is expected when Letta is not running."""
    ...
    def __repr__(self) -> str:
        return (f"MemoryManager(working={type(self.working).__name__}, "
